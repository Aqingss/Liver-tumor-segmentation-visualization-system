...
from Attention_UNet import AttentionUNet  # 导入Attention_UNet模型
import io
...
# 模型初始化 - 使用Attention_UNet
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model = AttentionUNet(in_channels=1, num_classes=4)  # 使用Attention_UNet替代普通UNet
model.load_state_dict(torch.load('models_attention/best.pth', map_location=device))  # 不同的模型路径
model.to(device)
model.eval()
...
def home():
    return """
    <h1>医学图像分割API(Attention_UNet版)</h1>
    <p>可用端点:</p>
    <ul>
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5002, debug=True)  # 使用5002端口