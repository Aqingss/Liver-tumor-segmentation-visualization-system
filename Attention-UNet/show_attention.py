...
# 2. 加载模型（灰度输入）
model = AttentionUNet(in_channels=1, num_classes=4)
model.load_state_dict(torch.load('models_attention/best.pth', map_location=device))
model.to(device)
model.eval()

# 3. 加载灰度图像
img_path = r'D:\VSpython\Liver_UNet\volume-100_567.png'
